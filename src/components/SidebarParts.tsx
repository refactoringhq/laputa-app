...
export function isSelectionActive(
  ...
  switch (check.kind) {
    ...
    case "view":
      return (current as typeof check).filename === check.filename;
    default:
      return false;
  }
}
...
function getNavItemPadding(compact: boolean | undefined, hasCount: boolean) {
  if (compact)
    return hasCount
      ? SIDEBAR_ITEM_PADDING.compactWithCount
      : SIDEBAR_ITEM_PADDING.compact;
  return hasCount
    ? SIDEBAR_ITEM_PADDING.withCount
    : SIDEBAR_ITEM_PADDING.regular;
}
...
function NavItemLabel({
  ...
  return (
    <span className={cn("flex-1 font-medium", getNavItemTextClass(compact))}>
      {label}
    </span>
  );
}
  ...
  return (
    ...
    >
      <SidebarNavIcon
        Icon={Icon}
        emoji={emoji}
        iconSize={getNavItemIconSize(compact)}
      />
...
export function SectionContent({
  ...
  const { sectionColor, sectionLightColor } = resolveSectionColors(
    type,
    customColor,
  );
  ...
  return (
    <SectionHeader
      ...
      onStartRename={() => onStartRename(type)}
      onSelectTypeNote={() => onSelectTypeNote(type)}
...
function getSectionHeaderBackground(
  isActive: boolean,
...
function getSectionSelectHandler(
...
function SectionHeaderCountPill({
  ...
  return (
    <SidebarCountPill
      ...
      style={
        isActive
          ? { background: sectionColor, color: "var(--text-inverse)" }
  ...
  return (
    ...
    >
      <div className="flex min-w-0 flex-1 items-center" style={{ gap: 4 }}>
        <Icon
          size={16}
          weight={getSectionHeaderIconWeight(isActive)}
          style={{ color: sectionColor, flexShrink: 0 }}
        />
        <SectionHeaderLabel
          type={type}
          label={label}
