...
function isDuplicateNativeKeydown(event: SearchKeyboardEvent, previous: SearchKeydownRecord | null): boolean {
...
function useSearchKeyboard({
  ...
  useSearchKeyboardDocumentListeners({
    handleKeyDown,
    handleKeyUp,
    open,
    pressedKeysRef,
...
export function SearchPanel({ open, vaultPath, entries, onSelectNote, onClose }: SearchPanelProps) {
  ...
  const {
      ...
      return (
        <div ref={rootRef} className="fixed inset-0 z-[1000] flex justify-center bg-[var(--shadow-dialog)] pt-[15vh]">
          <button
            type="button"
            ...
            onClick={onClose}
          />
          <div className="relative z-10 flex w-[540px] max-w-[90vw] max-h-[480px] flex-col self-start overflow-hidden rounded-xl border border-[var(--border-dialog)] bg-popover shadow-[0_8px_32px_var(--shadow-dialog)]">
            <SearchInput ref={inputRef} query={query} loading={loading} onChange={setQuery} />
            <SearchContent
              query={query}
              results={results}
