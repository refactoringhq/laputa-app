...
const PROPERTY_ROW_CLASS_NAME =
  'group/prop grid min-h-7 min-w-0 grid-cols-2 items-center gap-2 rounded px-1.5 outline-none transition-colors hover:bg-muted focus:bg-muted focus:ring-1 focus:ring-primary'

function PropertyRow(options: {
  propKey: string
      ...
      <span className={PROPERTY_PANEL_LABEL_CLASS_NAME}>
        ...
        {onDelete && (
          <button
            ...
            title={translate(locale, 'inspector.properties.deleteProperty')}
          >
            &times;
          </button>
        )}
  ...
  return (
    ...
    >
      <span className={PROPERTY_PANEL_PLACEHOLDER_LABEL_CLASS_NAME}>
        <span className={PROPERTY_PANEL_LABEL_ICON_SLOT_CLASS_NAME} data-testid="add-property-icon-slot">
...
function SuggestedPropertySlot({
  label,
  displayMode,
  onAdd,
}: {
  ...
  return (
    ...
    >
      <span className={PROPERTY_PANEL_PLACEHOLDER_LABEL_CLASS_NAME}>
        <span className={PROPERTY_PANEL_LABEL_ICON_SLOT_CLASS_NAME} data-testid="suggested-property-icon-slot">
          <SuggestedIcon
...
function TypeDerivedPropertySlot(options: {
  propKey: string
  editingKey: string | null
  displayMode: PropertyDisplayMode
  autoMode: PropertyDisplayMode
  ...
  return (
    ...
    >
      <span className={PROPERTY_PANEL_PLACEHOLDER_LABEL_CLASS_NAME}>
        <span className={PROPERTY_PANEL_LABEL_ICON_SLOT_CLASS_NAME} data-testid="type-derived-property-icon-slot">
          <PlaceholderIcon
            className="size-3.5 shrink-0 text-muted-foreground/40"
...
function getExistingPropertyKeys(
...
function getMissingSuggestedProperties(
  ...
  return SUGGESTED_PROPERTIES.filter(({ key }) => !existingKeys.has(key.toLowerCase()) && key !== pendingSuggestedKey)
}
...
function useSuggestedPropertyActions({
  ...
  const handlePendingSuggestedEdit = useCallback(
    (key: string | null) => {
    setEditingKey(key)
  ...
  const handleSaveSuggestedValue = useCallback(
...
function PropertyEntryRows(options: {
...
function PendingSuggestedPropertyRow(options: {
  pendingSuggestedKey: string | null
  editingKey: string | null
  vaultStatuses: string[]
  vaultTagsByKey: Record<string, string[]>
...
function MetadataSelectors(options: {
...
function DynamicPropertiesPanelContent(options: {
  entry: VaultEntry
  ...
  const {
        ...
        vaultTagsByKey,
        propertyEntries,
        typeDerivedPropertyEntries,
        handleSaveValue,
        handleSaveTypeDerivedValue,
        handleSaveList,
        handleAdd,
      ...
      return (
        <div className="flex flex-col gap-3">
          <div className="grid min-w-0 gap-x-2 gap-y-1.5" style={PROPERTY_PANEL_GRID_STYLE}>
            ...
            {!showAddDialog && (
              <AddPropertyButton locale={locale} onClick={() => setShowAddDialog(true)} disabled={!onAddProperty} />
            )}
          </div>
          {showAddDialog && (
            <AddPropertyForm
  ...
  const missingTypeName = useMemo(
    () => resolveMissingTypeName(entry.isA, propertyState.availableTypes),
    [entry.isA, propertyState.availableTypes],
  )
  const format = useMemo(() => noteFormatFromFrontmatter(frontmatter), [frontmatter])

  const existingKeys = useMemo(
    () =>
      getExistingPropertyKeys(
        [...propertyState.propertyEntries, ...propertyState.typeDerivedPropertyEntries],
        frontmatter,
      ),
  ...
  const { handlePendingSuggestedEdit, handleSaveSuggestedValue, handleSuggestedAdd } = useSuggestedPropertyActions({
    onAddProperty,
    setEditingKey: propertyState.setEditingKey,
    setPendingSuggestedKey,
  })

  useFocusNoteIconProperty({
    onAddProperty,
    setEditingKey: propertyState.setEditingKey,
    setPendingSuggestedKey,
  })
