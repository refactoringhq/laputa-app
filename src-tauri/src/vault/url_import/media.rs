...
pub(super) const MAX_MEDIA_BYTES: u64 = 25 * 1024 * 1024;
...
fn parse_markdown_destination(bytes: &[u8], start: usize) -> Option<MarkdownDestination> {
    ...
    for (offset, byte) in bytes[destination_start..].iter().enumerate() {
        ...
        match *byte {
            ...
            b')' => paren_depth -= 1,
            byte if byte.is_ascii_whitespace() && paren_depth == 0 => {
...
pub(super) fn save_remote_media(
    client: &Client,
    vault_path: &Path,
    note_slug: &str,
    index: usize,
...
pub(super) fn validate_media_url_for_fetch(media_url: &Url, page_url: &Url) -> Result<(), String> {
...
pub(super) fn validate_media_response_peer(
    response: &Response,
    page_url: &Url,
) -> Result<(), String> {
    if response
