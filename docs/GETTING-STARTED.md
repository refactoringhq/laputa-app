...
5. **OpenCode compatibility**: Keep `~/.config/opencode/opencode.json` in the registration path; OpenCode uses a different entry format (`"mcp"` key, `"command"` as array, `"environment"` for env vars, `"type": "local"`, `"enabled": true`) handled by `build_opencode_mcp_entry()` and `upsert_mcp_config_with_key()`
6. **Stable MCP path for AppImage**: On Linux AppImage launches, `extract_mcp_server_to_stable_dir(app_version)` copies `mcp-server/` to `~/.local/share/tolaria/mcp-server/` so external registrations reference a stable path. The copy is gated by a `.tolaria-version` marker file compared against the Tauri app version — it only runs on first launch or after an app update
7. **Process lifecycle**: Stdio MCP servers in `mcp-server/index.js` must exit when their external client closes stdin, and the desktop-owned `ws-bridge.js` child must be stopped on vault deselection, vault switch, and app exit
