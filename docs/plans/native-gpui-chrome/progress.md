...
| 8.x — Behavioral fidelity pass (Strand A — stub completion) | ⏳ in progress | — | — | Wire stubbed handlers in `sidebar_panel`, `note_list_pane`, `note_item`, `inspector_panel`, `search_panel`, `status_bar`, `breadcrumb_bar`, `actions`, `banners`, `toasts`, `vault`, `theme`, `workspace`, `settings_panel` (8.1–8.14). |
| 8.x — Behavioral fidelity pass (Strand B — missing surfaces) | ⏳ planned | — | — | New crates: `frontmatter_panel`, `raw_editor`, `folder_tree`, `filter_builder`, `workspace_switcher`, `note_retargeting`, `rendering_primitives`, `onboarding_prompts`, `ai_panel` wiring (8.15–8.23). |
