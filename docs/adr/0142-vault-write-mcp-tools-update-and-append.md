...
- `vault.js`: `updateNote(vaultPath, notePath, content, { expectedMtime })` and `appendToNote(vaultPath, notePath, content)`. Both reuse `resolveVaultNotePath` for the same traversal guard as `getNote`. `updateNote` writes atomically (temp file alongside the target, then `rename`) so a crashed write never leaves a truncated note; `appendToNote` uses `open(..., 'a')` for a single in-place append and returns the resulting `mtimeMs`.
