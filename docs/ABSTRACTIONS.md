...
- **PDF imports** — `pdf_markdown_import_started`, `pdf_markdown_import_completed`, and `pdf_markdown_import_failed` report the trigger source, conversion mode, page count, OCR page count, and text-length bucket. They never include PDF paths, note paths, titles, or extracted text.
