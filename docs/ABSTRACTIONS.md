...
- The `/` slash menu remains the supported path for markdown-safe block transformations such as headings, quotes, list blocks, Mermaid diagrams, whiteboards, and BlockNote-native two/three column layouts. Tolaria filters out BlockNote's toggle-heading and toggle-list variants because those do not map cleanly to the markdown note model.
- `@blocknote/xl-multi-column` extends the main editor schema with `columnList` and `column` blocks, contributes the localized two/three column slash commands, and replaces the default drop cursor with `multiColumnDropCursor` so dragged blocks can land beside existing columns. Tolaria maps its app locale to the closest BlockNote and multi-column dictionaries, with English fallback for app locales the extension does not ship.
