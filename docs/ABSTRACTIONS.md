...
`SingleEditorView` contains recoverable BlockNote render failures at the editor boundary. A recoverable error is marked during React's derived error-state phase so root telemetry can recognize it even when production minification removes component names; the boundary remounts the editor once, then keeps the surrounding vault UI mounted if the same failure persists. Rich-editor wikilink, emoji, and slash-menu item hooks live in `singleEditorSuggestionItems.tsx`, while whitespace selection and container-click behavior live in `singleEditorPointerInteractions.ts`; both keep their memoized callbacks stable without adding unrelated complexity to the render boundary.
...
Graph View is the first native graph surface built on the same renderer-owned entry graph. `src/components/GraphView/graphModel.ts` adapts mounted `VaultEntry` objects into real and ghost nodes, aggregates body-link and relationship evidence, and derives degree, PageRank, connected communities, and neighborhood traversal without a local HTTP service. The 2D/3D force renderers and Cytoscape analysis renderer consume that pure payload; Tolaria remains responsible for locale, theme, workspace provenance, and opening the selected note.
...
- `src/components/codeBlockLanguageControls.tsx` owns the visible shadcn language picker. It tracks each live BlockNote code block by block ID, overlays the library-owned selector, and writes changes through the editor API so a selector created during a transient read-only render cannot remain stale or disabled.
...
- `SingleEditorView` disables BlockNote's default formatting toolbar, `/` menu, side menu, and file panel, then mounts Tolaria-owned controllers so the visible formatting surface matches Tolaria's markdown round-trip guarantees. `TolariaFilePanel` reuses BlockNote's existing file/image UI only while its target block still exists; note reloads and untitled renames therefore close a stale attachment panel instead of rendering it against missing block state.
