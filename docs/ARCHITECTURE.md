...
#### External Capture Deep Links

Tolaria owns the desktop receiver side of external capture handoffs. Browser or OS capture adapters put large/private Markdown bodies on the clipboard and open a small control URL such as `tolaria://clip/new?v=1&clipboard=1&path=Clippings%2FExample.md&title=Example`; the URL never carries the note body. The renderer accepts only the supported `clip/new` v1 clipboard contract, rejects body-bearing query params, validates the requested path as vault-relative and non-traversing, then writes and opens the note through the same disk-first note creation path as in-app creation.

On macOS, custom scheme support is not an Apple registration step. The released `Tolaria.app` bundle must advertise `tolaria` in `Contents/Info.plist` through `CFBundleURLTypes`, and `setup_desktop_plugins()` must initialize `tauri_plugin_deep_link::init()` so LaunchServices-delivered URLs reach the app. Installing or updating Tolaria is what gives macOS a bundle to register; an older `/Applications/Tolaria.app` without the scheme remains an unknown-protocol handler failure even if the repository source or a separate local build already contains the deep-link code.

Release QA for scheme changes therefore checks the built app artifact, not just source config:

```bash
/usr/libexec/PlistBuddy -c 'Print :CFBundleURLTypes' \
  /Applications/Tolaria.app/Contents/Info.plist

/System/Library/Frameworks/CoreServices.framework/Frameworks/LaunchServices.framework/Support/lsregister \
  -f /Applications/Tolaria.app

open 'tolaria://clip/new?v=1&clipboard=1&path=Clippings%2FDeepLinkSmoke.md&title=DeepLinkSmoke'
```

The first command must show `CFBundleURLSchemes = tolaria`; the `open` smoke must launch or focus Tolaria rather than reporting an unknown protocol. When an active vault and clipboard smoke Markdown are present, the note should be created, reloaded into the vault list, and opened in the editor.
