...
Window focus also requests Git-based external rename detection for the active vault. The `detect_renames` Tauri command runs its filesystem and Git subprocess work on Tokio's blocking pool so a slow repository cannot stall the native event loop while the user resumes typing.
