...
} from "./blockNoteSideMenuHoverGuard.ts";

// Ported from `src/components/blockNoteSideMenuHoverGuard.test.ts`
// (Phase 8.25 — editor-host slash/side/formatting menus row).  The
// React-side suite asserts the bridge geometry and event suppression;
// happy-dom's DOMRect implementation matches the browser here, so the
// only adjustment is the import path.
