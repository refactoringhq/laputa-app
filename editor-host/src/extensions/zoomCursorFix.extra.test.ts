...
describe("zoomCursorFix extra coverage", () => {
    ...
    it("uses caretRangeFromPoint for zoomed coordinates and restores prototype methods on destroy", () => {
        const protoPosAtCoords = vi
            .spyOn(EditorView.prototype, "posAtCoords")
            .mockReturnValue(99);
        const protoPosAndSideAtCoords = vi
            .spyOn(EditorView.prototype, "posAndSideAtCoords")
            .mockReturnValue({ pos: 99, assoc: -1 });
        ...
        expect((view as unknown as {
            posAtCoords: (c: { x: number; y: number }, precise?: boolean) => number | null;
        }).posAtCoords({ x: 20, y: 30 }, true)).toBe(3);
        expect(
            (
                view as unknown as {
                    posAndSideAtCoords: (
            ...
            ).posAndSideAtCoords({ x: 20, y: 30 }, false),
        ).toEqual({ pos: 3, assoc: 1 });
    ...
    it("falls back to original coordinate methods when zoom is 1 or caret lookup misses", () => {
        ...
        expect((view as unknown as {
            posAtCoords: (c: { x: number; y: number }, precise?: boolean) => number | null;
        }).posAtCoords({ x: 8, y: 12 }, true)).toBe(7);
        ...
        expect(
            ...
            ).posAndSideAtCoords({ x: 20, y: 30 }, true),
        ).toEqual({ pos: 11, assoc: -1 });
