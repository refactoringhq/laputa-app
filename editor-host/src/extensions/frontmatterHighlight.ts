...
/**
 * The optional `_dark` argument is unused but kept for parity with the
 * React-side signature — both light- and dark-mode color tokens are
 * resolved through CSS variables (`--syntax-frontmatter-key`,
 * `--syntax-frontmatter-value`) so the theme switch happens in the
 * stylesheet, not the extension.
 */
export function frontmatterHighlightTheme(_dark?: boolean) {
    return EditorView.baseTheme({
        ".cm-frontmatter-delimiter": {
            color: "var(--syntax-frontmatter-key)",
            fontWeight: "600",
        },
