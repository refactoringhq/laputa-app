// ---------------------------------------------------------------------------
// compactMarkdown — BlockNote serialiser output normaliser
// ---------------------------------------------------------------------------
//
// Ported verbatim from `src/utils/compact-markdown.ts` so the editor-
// host's `save_request` body matches the React variant byte-for-byte
// (worklist 2.26 follow-up).
//
// BlockNote's `blocksToMarkdownLossy` emits:
// - `*` bullet markers (we standardise on `-`)
// - blank lines between consecutive list items (we tighten them)
// - HTML entity escapes like `&#x20;` for spaces around bold (we decode)
// - leading/trailing inline whitespace inside `**…**` markers (we
//   shift the whitespace outside the markers so the rendered text is
//   identical)
// - stray backslash-only hard-break lines after an in-text hard break
// - runs of 3+ blank lines (we collapse to one blank line)
// - a trailing blank line (we strip it but keep exactly one trailing
//   newline)
//
// Code blocks are left untouched — anything inside a fenced block is
// content, not markdown structure.
//
// React tests at `src/utils/compact-markdown.test.ts` translate
// verbatim; keep both in lockstep so the two variants don't drift.
...
/** True if this blank line is part of a run of 2+ consecutive blank
 *  lines (i.e. would create 3+ newlines in a row — collapse to just
 *  one blank line). */
