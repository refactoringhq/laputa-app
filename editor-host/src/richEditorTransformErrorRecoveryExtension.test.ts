...
} from "./richEditorTransformErrorRecoveryExtension.ts";
...
function createView(error?: Error) {
    ...
    // `dispatch` accepts a transaction-shaped argument so the test
    // can pass `{ before: ... }` envelopes through the recovering
    // wrapper without TypeScript narrowing the parameter list to `[]`.
    const dispatch = vi.fn((_transaction?: unknown) => {
