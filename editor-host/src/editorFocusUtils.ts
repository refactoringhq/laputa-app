// ---------------------------------------------------------------------------
// Editor focus utilities (ADR-0115 Phase 8.30)
// ---------------------------------------------------------------------------
//
// Ported verbatim from `src/hooks/editorFocusUtils.ts` (React reference).
// The implementation is DOM-only — no Tauri / vault dependencies — so
// the editor-host can carry the title-select-on-create + retry-on-
// late-mount behaviours without modification.
//
// The selectors and constants are kept identical to the React app so the
// `useEditorFocus` hook + its tests can be ported verbatim too.
...
export interface TiptapEditor {
    state: {
        doc: {
            descendants: (
                cb: (node: { type: { name: string }; nodeSize: number }, pos: number) => boolean | void,
            ) => void;
        };
    };
    chain: () => TiptapChain;
...
export function focusEditorWithRetries(
    ...
    // The first heading can arrive a frame or two later than the
    // initial focus on slower CI and native tab-swap paths, so keep
