// ---------------------------------------------------------------------------
// Editor focus orchestration (ADR-0115 Phase 8.30)
// ---------------------------------------------------------------------------
//
// Ported verbatim from `src/hooks/useEditorFocus.ts` (React reference).
// Listens for `laputa:focus-editor` window events and routes them to the
// editor surface, optionally selecting the first H1 (used by
// "new note → focus title" UX).
//
// The custom-event names match the React app's so the editor-host stays
// drop-in compatible if the native shell ever dispatches them through
// `WebView::evaluate_script("window.dispatchEvent(...)")`.

import { useEffect } from "react";
import { focusEditorWithRetries, type FocusableEditor } from "./editorFocusUtils.ts";
...
export function useEditorFocus(
    ...
    useEffect(() => {
        ...
        const handler = (e: Event) => {
            ...
            const scheduleFocus = () =>
                scheduleEditorFocus(editor, editorMountedRef, selectTitle, t0);
